...
public class BagDraftSettings extends SettingsMenu {
    // ---------------------------------------------------------------------------------------------------------------------------------
    // Settings & Submenus
    // ---------------------------------------------------------------------------------------------------------------------------------
    private final BooleanSetting presetDraftOrder;
    private final IntegerSetting picksPerRound;
    private final IntegerSetting picksOnFirstRound;
    ...
    private static final String MENU_ID = "dsBag";

    public BagDraftSettings(Game game, JsonNode json, SettingsMenu parent) {
        super(MENU_ID, "Bag Draft", "Control how the bag draft works", parent);

        // Initialize Settings to default values
        presetDraftOrder = new BooleanSetting("Static Order?", "use static order", false);
        picksPerRound = new IntegerSetting("Picks / Round", "picks per round", 2, 1, 5, 1);
        picksOnFirstRound = new IntegerSetting("1st Round Picks", "picks on first round", 3, 1, 5, 1);
        ...
        if (!(json == null
            ...
            picksPerRound.initialize(json.get("picksPerRound"));
            picksOnFirstRound.initialize(json.get("picksOnFirstRound"));
    ...
    public List<SettingInterface> settings() {
        ...
        ls.add(picksPerRound);
        ls.add(picksOnFirstRound);
        return ls;
    }
